...
namespace FineCollectionService.Tests
{
    public class FineCollectionServiceUnitTests
    {
        ...
        [Fact]
        public async Task RetreiveLicenseKey() 
        {
            client.DefaultRequestHeaders.Accept.Clear();
            client.DefaultRequestHeaders.Accept.Add(
                new MediaTypeWithQualityHeaderValue("application/json")
            ...
            Stream streamTask;
            try {
                streamTask = await client.GetStreamAsync("http://localhost:3601/v1.0/secrets/trafficcontrol-secrets/finecalculator.licensekey");
            }
            catch (Exception ex) {
                throw new XunitException($"Unable to query endpoint. Error: {ex.Message}");
            }
            ...
            Assert.NotEmpty(actualResult.RootElement.GetProperty("finecalculator.licensekey").GetString());
        }

        [Fact]
        public async Task RetreiveSmtpUsernameKey() 
        {
            client.DefaultRequestHeaders.Accept.Clear();
            client.DefaultRequestHeaders.Accept.Add(
                new MediaTypeWithQualityHeaderValue("application/json")
            ...
            Stream streamTask;
            try {
                streamTask = await client.GetStreamAsync("http://localhost:3601/v1.0/secrets/trafficcontrol-secrets/smtp.username");
            }
            catch (Exception ex) {
                throw new XunitException($"Unable to query endpoint. Error: {ex.Message}");
            }
            ...
            Assert.NotEmpty(actualResult.RootElement.GetProperty("smtp.username").GetString());
        }

        [Fact]
        public async Task RetreiveSmtpPasswordKey() 
        {
            client.DefaultRequestHeaders.Accept.Clear();
            client.DefaultRequestHeaders.Accept.Add(
                new MediaTypeWithQualityHeaderValue("application/json")
            ...
            Stream streamTask;
            try {
                streamTask = await client.GetStreamAsync("http://localhost:3601/v1.0/secrets/trafficcontrol-secrets/smtp.password");
            }
            catch (Exception ex) {
                throw new XunitException($"Unable to query endpoint. Error: {ex.Message}");
            }
            ...
            Assert.NotEmpty(actualResult.RootElement.GetProperty("smtp.password").GetString());
        }
    }
}
